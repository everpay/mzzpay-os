...
describe("settlement-meta contract", () => {
  ...
  it("card payments settle T+2, APMs T+1", () => {
    expect(settlementDaysFor("card")).toBe(2);
    expect(settlementDaysFor("CARD_VISA")).toBe(2);
  ...
  it("expected_settlement_at = received_at + settlementDays", () => {
    ...
    expect(new Date(cardMeta.expected_settlement_at).getTime() - NOW.getTime()).toBe(2 * 86_400_000);
...
describe("deriveBadge — UI mirrors webhook output", () => {
  ...
  it("end-to-end: webhook writes meta → UI derives the matching badge", () => {
    ...
    // T+2d (exactly window) → still scheduled
    expect(deriveBadge({ _risonpay_meta: meta }, "completed", new Date(received.getTime() + 2 * 86400_000))).toBe("scheduled");
    // T+2d + 7h → delayed
    expect(deriveBadge({ _risonpay_meta: meta }, "completed", new Date(received.getTime() + 2 * 86400_000 + 7 * 3600_000))).toBe("delayed");
  });
});
